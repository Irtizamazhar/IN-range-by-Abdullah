...
export const accountDestinations = [
  ...
  ["help","Help & Support","/account/help"],
] as const;
export function AccountNavigation() {
  const pathname = usePathname(); const search = useSearchParams();
  const active = pathname === "/account" ? search?.get("tab") || "overview" : pathname === "/my-stuff" ? "returns" : pathname?.startsWith("/account/orders/") ? "orders" : pathname?.startsWith("/account/help") ? "help" : pathname?.split("/").pop();
