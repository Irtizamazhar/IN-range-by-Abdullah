...
export async function checkoutQuote(tx: Prisma.TransactionClient, quoteId: string, customerEmail: string, city: string) {
  const quote = await tx.offerQuote.findFirst({ where: { id: quoteId, customer: { email: customerEmail.trim().toLowerCase() } }, include: { revision: { include: { offer: { select: { status: true, vendorId: true } } } }, want: { select: { status: true, expiresAt: true, city: true } } } });
