...
async function main() {
  ...
  const anonymous=await browser.newContext(); const anonPage=await anonymous.newPage(); await anonPage.goto(`${base}${invite.invitePath}`); await anonPage.getByRole("button",{name:"Join room",exact:true}).click(); await anonPage.getByRole("dialog").waitFor(); await anonymous.close(); pass("anonymous invite opens sign-in without leaking room contents");
