...
export async function PATCH(request: Request, { params }: Context) { return api(async () => {
  ...
  return prisma.$transaction(async tx => {
    const offer = await tx.wantOffer.findUnique({ where: { id: params.id }, include: { want: true, vendor: { select: { status: true } }, revisions: { orderBy: { revisionNumber: "desc" }, take: 1 } } });
    ...
    if (action === "accept") {
      ...
      await tx.wantOffer.update({ where: { id: offer.id }, data: { status: "ACCEPTED" } }); return { quote };
    }
    if (action === "withdraw" || action === "reject") return { offer: await tx.wantOffer.update({ where: { id: offer.id }, data: { status: action === "withdraw" ? "WITHDRAWN" : "REJECTED" } }) };
    if (action === "counter" && latest.initiator !== "VENDOR") throw new ApiError(409, "Wait for a vendor response.");
    const terms = offerTerms.parse(body); validateOfferExpiry(new Date(terms.expiresAt));
    ...
    return { offer: await tx.wantOffer.update({ where: { id: offer.id }, data: { version, status: vendor ? "SUBMITTED" : "COUNTERED" } }) };
