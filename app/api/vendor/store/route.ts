...
const input = z.object({ shopName: z.string().trim().min(1).max(160), storeSlug: z.string().trim().toLowerCase().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/).min(3).max(80), shopDescription: z.string().max(5000), shopLogo: image, storeBanner: image, storePolicies: z.string().max(5000), primaryCategory: z.string().trim().min(1).max(100), serviceCities: z.array(z.string().trim().min(1).max(100)).max(100) });
