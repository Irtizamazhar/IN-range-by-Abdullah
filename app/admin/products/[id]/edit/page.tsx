...
export default function EditProductPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/products/${id}`)
      ...
      .then((p) => {
        ...
        const urls = (p.images as string[] | undefined) || [];
        setImages(
          ids.map((imgId, i) => ({
            id: imgId,
            url: urls[i] || `/api/image/${imgId}`,
          }))
        );
        setReady(true);
      });
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        toast.error(String(err?.error || "Could not update product"));
        return;
      }
  ...
  function onPasteImages(e: ClipboardEvent) {
    ...
    if (!pasted.length) {
      toast.error("No image in clipboard — copy a picture first (e.g. right-click → Copy image)");
      return;
    }
    e.preventDefault();
  ...
  return (
    <div className="p-6 md:p-8 max-w-2xl">
      ...
      <div className="mt-4 mb-6">
        <h1 className="text-2xl font-bold text-darkText">Edit Product</h1>
      </div>
      <form onSubmit={onSubmit} className="space-y-4 rounded-card border border-borderGray bg-white p-6 shadow-card">
        ...
        <div
          ...
          className="rounded-xl border-2 border-dashed border-primaryBlue/40 bg-primaryBlue/5 p-4 text-center outline-none focus:ring-2 focus:ring-primaryBlue/50 sm:p-6"
        >
